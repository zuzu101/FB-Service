...
class NotaService
{
    ...
    public function data(object $deviceRepair)
    {
        ...
        foreach ($array as $item) {
            ...
            $nestedData['actions'] = '
                <div class="btn-group">
                    <a href="' . route('admin.MasterData.Nota.print', $item->id) . '" class="btn btn-outline-info btn-sm" target="_blank"><i class="fa fa-print"></i> Print</a>
                    <a href="' . route('admin.MasterData.Nota.pdf', $item->id) . '" class="btn btn-outline-danger btn-sm" target="_blank"><i class="fa fa-file-pdf"></i> PDF</a>
                </div>
            ';
